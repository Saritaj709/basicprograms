...
File fileDoctor=new File("/home/bridgelabz/JSarita/ClinicalManagementSystem/src/com/bridgelabz/files/Doctor.json");
List<Doctor> doctorList=new LinkedList<>();
	...
	public void showDoctorDetails() throws JsonParseException, JsonMappingException, IOException {
		...
		for(int index=0;index<doctorList.size();index++)
		{
		 ...
		 System.out.println("Id of Doctor "+doctorList.get(index).getDoctorName()+" is "+doctorList.get(index).getDoctorId());
		 System.out.println("Specialization of Doctor "+doctorList.get(index).getDoctorName()+" is "+doctorList.get(index).getDoctorSpecialization());
		System.out.println("Availability of Doctor "+doctorList.get(index).getDoctorName()+" is "+doctorList.get(index).getDoctorAvailability());
		System.out.println("Patient count of Doctor "+doctorList.get(index).getDoctorName()+" is "+doctorList.get(index).getPatientCountForDoctor());
		}
	}
